...
static int startEntityId = 300;
	...
	public void init(FMLInitializationEvent event) {
		...
		//proxy.registerRenderers();// calls the methods in our proxy, which will
									// do things on client side

		LanguageRegistry.instance().addStringLocalization(
				"entity.Robot.name", "en_US", "Robot");// set a more
																	// readable
																	// name for
																	// the
																	// entity in
	...
	public static void registerEntityEgg(Class <? extends Entity> entity, int primaryColor, int secondaryColor) {
	  ...
	  @EventHandler // used in 1.6.2 //@Init // used in 1.5.2 
	  public void load(FMLInitializationEvent event) { 		  
		  proxy.registerRenderers(); 
		  
		  EntityRegistry.registerGlobalEntityID(EntityRobot.class, "Robot", 1);
		  EntityRegistry.addSpawn(EntityRobot.class, 10, 2, 4, EnumCreatureType.creature);
		  EntityRegistry.findGlobalUniqueEntityId();
		  registerEntityEgg(EntityRobot.class,0xffffff, 0x000000);
//		  RenderingRegistry.registerEntityRenderingHandler(entityRobot.class, new RenederRobot(new ,.5f) renderer);
		  }
